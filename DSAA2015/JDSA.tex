...
\end{figure}
%
%\footnote{Figure~\ref{fig:attributes} is using registration as well.} 
...
Figure~\ref{fig:meta-query} shows an example of a metaquery for the university database.\footnote{Zhensong: should we make this table a latex table? The font is small in any case, maybe make it table. \textbf{Oliver}: actually we tried the latex table before which does NOT fit well.
...
\KwIn{Database $\D$; first-order random variables $\F$; relationship chain lattice $\lattice$ with maximum chain length $\it{m}$.}
...
\end{equation}
 ...
 A relationship chain $\set{\Relation}$ is a \defterm{subchain} of another chain $\set{\Relation'}$, written $\set{\Relation} \sqsubseteq \set{\Relation'}$, if every relationship variable in $\set{\Relation}$ occurs also in $\set{\Relation'}$. For example, the chain~(\ref{eq:rchain1}) is a subchain of the chain~(\ref{eq:rchain2}).
...
\subsubsection{Computing Contingency Tables}
