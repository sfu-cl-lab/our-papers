...
\journalname{Int. J. Data  Science and Analytics}
%
\begin{document}
...
\begin{abstract}
We describe \FB, a new SQL-based framework that leverages a relational database management system to support multi-relational model discovery. A multi-relational statistical model provides an integrated analysis of the heterogeneous and interdependent data resources in the database.  We adopt the BayesStore design philosophy: statistical models are stored and managed as first-class citizens inside a database \cite{Wang2008}. Whereas previous systems like BayesStore support multi-relational inference,  {\sc FactorBase} supports multi-relational learning. 
...
\keywords{Relational Learning \and Bayesian Networks \and Model Selection}
...
The relational schema for the Parameter Manager is shown in Table~\ref{table:mdb-schema}. The @par-RVID@ parameter refers to the ID  of a par-RV, for instance $\it{Capability}(\P,\S)$.
...
For each node, the $\MDB$ manages a conditional probability table. This is a factor table that represents the factor associated with the node's family (see Table~\ref{tab:pbn-cp}).
%
...
the $\it{RA}$ relationship only. For each relationship chain, the figure shows the complete Bayesian network structures learned, and excerpts from the contingency tables.}
 \label{fig:lja-trace}
 %original is in figures/graphs.pptx
\end{figure*}
...
\resizebox{0.5\textwidth}{!}{
\begin{tabular}[c]{ccc}
  \multicolumn{2}{c}{\it{LatticeMember}}\\
Member & Chain & \\\hline
RA(P,S) & [RA(P,S),Registered(Student,Course)]\\
Registered(Student,Course) &[RA(P,S),Registered(Student,Course)]\\\hline \hline
\multicolumn{2}{c}{\it{LatticeOrder}}\\
Subchain & SuperChain \\\hline
RA(P,S) & [RA(P,S),Registered(Student,Course)]\\
 Registered(Student,Course) & 
[RA(P,S),Registered(Student,Course)]
\end{tabular}
}
\end{center}
...
Rchain & child & parent \\\hline
 ...
 \it{RA(P,S)} & Salary(P,S) & Capability(P,S) \\\hline\hline
& \it{EntityBayesNets} & \\
FOVariable & child & parent\\\hline
...
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)}
\end{tabular}& Difficulty(C) &  Level(C)\\
\end{tabular}
\end{center}
