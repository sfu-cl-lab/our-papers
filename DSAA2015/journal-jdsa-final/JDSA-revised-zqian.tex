...
% helps todonotes come out right, should remove for final version
...
\title{FACTORBASE: Multi-Relational Structure Learning with SQL All The Way %\thanks{Grants or other notes
...
\begin{abstract}
\FB is a new SQL-based framework that leverages a relational database management system to support multi-relational model discovery. A multi-relational statistical model provides an integrated analysis of the heterogeneous and interdependent data resources in the database.  We adopt the BayesStore design philosophy: statistical models are stored and managed as first-class citizens inside a database \cite{Wang2008}. Whereas previous systems like BayesStore support multi-relational inference,  {\sc FactorBase} supports multi-relational learning. 
A case study on six benchmark databases evaluates how our system supports a challenging machine learning application, namely learning a first-order Bayesian network model for an entire database. Model learning in this setting has to examine a large number of potential statistical associations across data tables. Our implementation shows how the SQL constructs in \FB facilitate the fast, modular, and reliable development of  scalable model learning systems.

\keywords{Relational Learning \and Bayesian Networks \and Model Selection \and Relational Database Management Systems}
...
%\todo{fix overflow to next column}
...
A \textbf{par-factor} is a pair $\parfactor = (\parrvs,\potential)$, where $\parrvs$ is a set of par-RVs, and $\potential$ is a \defterm{potential function} from the values of the par-RVs to the non-negative real numbers\footnote{A par-factor can also include constraints on possible groundings.}. A \defterm{factor assignment} $\set{x}_\parrvs$ assigns a value to each par-RV in a parfactor. The factor potential function maps each factor assignment $\set{x}_\parrvs$ to a real number $\potential(\set{x}_\parrvs)$. 

A \defterm{factor grounding} assigns a constant to each first-order variable in the par-RV set $\parrvs$; the result is a ground par-factor $\gparrvs$.
Intuitively, a ground par-factor represents a set of random variables that interact with each other locally. SRL models use {\em parameter tying}, meaning that if two groundings of the same par-factor are assigned the same values, they return the same factor value. Let $\instantiations(\parfactor)$ denote the set of  all ground par-RVs in par-factor $\parfactor$. 
 ...
 A relational log-linear model is defined by a set of par-factors $\parfactors$. 
...
\item For each factor assignment, raise the potential function value $\potential(\set{x}_{\parrvs})$  to the number $n(\set{x}_{\parrvs})$ of ground factors that instantiate it in the database %defined by the assignment 
$\set{X}=\set{x}$ . 
...
%\begin{table}[htbp]
%  \centering
%  \caption{Database table instances: $\prof$.}
%    \begin{tabular}{|c|c|c|}
%
%    \multicolumn{3}{c}{\textbf{ Professor}} \\  \hline
%
...
%\begin{table}[htbp]
%  \centering
%  \caption{Database table instances:  $\ra$.}
%    \begin{tabular}{|c|c|c|c|}
%
%    \multicolumn{4}{c}{\textbf{ RA }} \\ \hline
%
...
We use the university example as a toy running example throughout the paper. The schema for the university domain is given in Figure~\ref{fig:university-schema}. This schema features only one relationship for simplicity; \FB learns a model for any number of relationships. While we describe \FB abstractly in terms of par-factors, for concreteness we illustrate it using Bayesian networks. The system takes as input a database instance like that shown in Table~\ref{tab:instances}, 
 %, Table~\ref{tab:instance2} and Table~\ref{tab:instance3},
...
\begin{table}[htbp]
  \centering
  \caption{To illustrate computing the log-likelihood of a  database instance according to Equation~\ref{eq:log-likelihood}. Each row corresponds to a factor assignment $\set{x}_{\parrvs}$. For each factor assignment, the equation requires multiplying the par-factor instantiation count, by the logarithm of the conditional probability. For example, for the assignment $\set{x}_{\parrvs} = \langle n/a, F, n/a \rangle$, we multiply the count $n(\set{x}_{\parrvs})= 203$, obtained from Table~\ref{tab:pbn-ct}, by the logarithm of the conditional probability 1, obtained from Table~\ref{tab:pbn-cp}. The value of this product is shown in the column $\ln$(CP)*Count (with 2 digits of precision). Equation~\ref{eq:log-likelihood} sums the values of these products for each factor assignment; in this example, the resulting sum $\sum_{\set{x}_{\parrvs}}$ equals $-26.42$.}
\resizebox{0.5\textwidth}{!}{
\begin{tabular}{|c|c|c|c|c|c|} \hline
\textbf{CP} & \textbf{Count} & \begin{tabular}{c}\bf{$\mathbf{\ln}$(CP)*}\\\bf{Count} \end{tabular} & \textbf{Capa(P,S)} & \textbf{RA(P,S)} & \textbf{Salary(P,S)} \\ \hline
1 & 203 & 0.00 & n/a & F & n/a \\ \hline
0.45 & 5 & -3.99 & 4 & T & high \\ \hline
0.36 & 4 & -4.09 & 5 & T & high \\ \hline
0.18 & 2 & -3.43 & 3 & T & high \\ \hline
0.2 & 1 & -1.61 & 3 & T & low \\ \hline
0.4 & 2 & -1.83 & 2 & T & low \\ \hline
0.4 & 2 & -1.83 & 1 & T & low \\ \hline
0.22 & 2 & -3.03 & 2 & T & med \\ \hline
0.44 & 4 & -3.28 & 3 & T & med \\ \hline
0.33 & 3 & -3.33 & 1 & T & med \\ \hline
        &    & $\sum -26.42$ & & & \\ \hline
\end{tabular}
}
  \label{tab:ll}%
...
\subsection{SRL Structure Learning} \label{sec:structure-learn}
...
\caption{FactorBase Process Diagram. All statistical objects are stored as first-class citizens in an RDBMS. Statistical objects are constructed and managed by three different modules. 1) The Variable Manager  constructs the random variable database, using an SQL script that analyzes the input database schema. 2) The Count Manager constructs contingency tables using the metadata in the VDB.  The contingency tables are constructed by count(*) queries, which themselves are constructed by SQL metaqueries. 3) The Model Manger constructs a relational graphical model with parameter estimates. It applies a graphical model search algorithm (specified by the user), which is supported by the metadata in the VDB, and by SQL queries against the contingency tables in the CDB. 
...
$\mathcal H$, score ($\cdot$, $\mathcal D$) )} 
	\STATE{ $\mathcal R \leftarrow $ \textsc{refine}C\textsc{andidates}($G, \mathcal H$)} \label{line:candidates}
 	\FORALL{$R \in \mathcal R$} 
		\STATE{$R \leftarrow$ \textsc{learn}P\textsc{arameters}($R$,score ($\cdot$, $\mathcal D$))} 
    \ENDFOR
...
\end{algorithmic}
\label{alg:learning}
\caption{Structure learning algorithm template
%(instantiation of procedures in lines 2, 3, 5 and 8 determines exact behavior) 
}
\end{algorithm}

\section{The Random Variable Database VDB} \label{sec:vdb}
...
% {vdb}
% }
% \caption{The Variable Manager  constructs the random variable database, using an SQL script that analyzes the input database schema.
% %[counts -> contingency table]
% %Here the $DB$ denotes the original database, $DB\_RV$ denotes the database storing all random variables, 
% %$DB\_CT$ denotes the database containing all the contingency tables  and $DB\_BN$ storing all the learned Bayes Nets.
% \label{fig:vdb}}
...
%A full description is provided in the Appendix. %~\ref{sec:appendix}.
...
\multicolumn{1}{|c|}{\textbf{ER Diagram} } &  \multicolumn{1}{|c|}{ \textbf{Example }} &\multicolumn{1}{|c|}{ \textbf{par-RV equivalent}} \\\hline
...
\begin{tabular}{|l|l|}
\hline
    \multicolumn{1}{|c|}{\textbf{Table Name}}&  \multicolumn{1}{|c|}{\textbf{Column Names}} \\\hline
{Relationship} & {RVarID: par-RV,  TABLE\_NAME: string} \\\hline
    ...
    {Relationship\_FOvariables} & {RVarID: par-RV, FO-ID1: FO-Var,  FO-ID2: FO-Var}\\\hline
        \end{tabular}%
        }
 \label{table:vdb-schema}
\end{table}
...
%\begin{table}[htbp]
%  \centering
%  \caption{ The metadata about attributes represented in \RVD database tables. The table $\it{AttributeColumns}$ specifies which tables and columns contain the functor values observed in the data. The column name is also the functor ID.}
%    \begin{tabular}{|l|l|}
%    \multicolumn{2}{c}{\textbf{AttributeColumns}} \\ \hline
...
%The details are provided in the SQL script shown in the Appendix~\ref{sec:appendix}. 
...
% {cdb}
% }
% \caption{The Count Manager constructs contingency tables using the metadata in the VDB.  The contingency tables are constructed by count(*) queries, which themselves are constructed by SQL metaqueries.
% %[counts -> contingency table]
% %Here the $DB$ denotes the original database, $DB\_RV$ denotes the database storing all random variables, 
% %$DB\_CT$ denotes the database containing all the contingency tables  and $DB\_BN$ storing all the learned Bayes Nets.
% \label{fig:cdb}}
...
Every set of par-RVs $\parrvs \equiv \{\parrv_{1},\ldots,\parrv_{n} \}$ has an associated \textbf{contingency table} ($\cttable$) denoted by $\cttable(\parrvs)$. %$CT(\set{V})$. 
This is a table with a row for each of the possible assignments of values to the variables in $\set{V}$, and a special integer column called $\qcount$. 
The value of the $\qcount$ column in a row 
...
\begin{table*}[thp]
\centering
\caption{Example of metaqueries and their results based on university database and the par-RV metadata (Table \ref{table:vdb-schema})}
\label{tab:meta-query}
\begin{tabular}{|l|l|}
\hline
\multicolumn{1}{|c|}{\textbf{Metaqueries} }        & \multicolumn{1}{c|} {\textbf{ Entries}} \\ \hline
\multirow{5}{*}{ \begin{tabular}{@{}l@{}}CREATE VIEW Select\_List AS \\
SELE RVarID, CONCAT(`COUNT(*)', ` AS ` ``COUNT" ') AS Entries \\
FROM VDB.Relationship
UNION DISTINCT \\
SELECT RVarID, AVarID as Entries \\
FROM VDB.Relationship\_Attributes; 
\end{tabular}} & COUNT(*) AS ``count"          \\ \cline{2-2} 
                  &   \textquotesingle  Popularity(P)\textquotesingle         \\ \cline{2-2} 
                  &   \textquotesingle  Teachingability(P)\textquotesingle           \\ \cline{2-2} 
                  &   \textquotesingle  Intelligence(S)\textquotesingle           \\ \cline{2-2} 
                  &    \textquotesingle  Ranking(S)\textquotesingle          \\ \hline
\multirow{6}{*}{
 \begin{tabular}{@{}l@{}}CREATE VIEW From\_List AS \\
SELECT RVarID, CONCAT (`@database@.',TABLE\_NAME) AS Entries \\
FROM VDB.Relationship\_FOvariables
UNION DISTINCT  \\
SELECT RVarID, CONCAT(`@database@.',TABLE\_NAME) \\ AS Entries 
FROM VDB.Relationship;\end{tabular}} &  \multirow{2}{*}{@database@.prof AS P   }      \\ 
& \\\cline{2-2} 
&  \multirow{2}{*}{@database@.student AS S    }        \\ 
& \\ \cline{2-2} 
&    \multirow{2}{*}{@database@.RA AS   \textquotesingle  RA\textquotesingle   }      \\ 
& \\ \hline

\multirow{3}{*}{
 \begin{tabular}{@{}l@{}}CREATE VIEW Where\_List AS  \\
SELECT RVarID, CONCAT(RVarID,`.', COLUMN\_NAME, `=', \\
FO\_ID, `.', REFERENCED\_COLUMN\_NAME) AS Entries \\
FROM VDB.Relationship\_FOvariables; \end{tabular}} &   \multirow{2}{*}{ \textquotesingle  RA\textquotesingle.p\_id = P.p\_id  }         \\ 
&\\ \cline{2-2} 
                  &     \multirow{2}{*}{ \textquotesingle  RA\textquotesingle.s\_id = S.s\_id    }      \\ 
& \\ \hline
\end{tabular}
\end{table*}
...
\section{The Model Manager MDB: Parameter Learning} \label{sec:model-manager}
...
% {mdb}
% }
% \caption{The Model Manger constructs a relational graphical model with parameter estimates. It applies a graphical model search algorithm (specified by the user), which is supported by the metadata in the VDB, and by SQL queries against the contingency tables in the CDB. 
% %[counts -> contingency table]
% %Here the $DB$ denotes the original database, $DB\_RV$ denotes the database storing all random variables, 
% %$DB\_CT$ denotes the database containing all the contingency tables  and $DB\_BN$ storing all the learned Bayes Nets.
% \label{fig:mdb}}
...
\subsection{Parameter Learning} \label{sec:parameters}
...
For learning the structure of a parametrized Bayesian network, we used \FB  to implement the previously existing learn-and-join algorithm (LAJ) \cite{Khosravi2010,Schulte2012}. The LAJ algorithm follows the general structure learning schema of Algorithm~\ref{alg:learning}. It uses a sophisticated strategy for generating candidate graphs (Line 3 of Algorithm~\ref{alg:learning}) that exploits the lattice nesting of relational paths.
...
two relationships, $\it{Registered}$ and $\it{RA}$. For each relationship chain, SQL meta queries compute a contingency table. Solid block lines: The inclusion relations among points in the relationship chain lattice. Solid arrows: For each relationship chain, a single table Bayesian network learner constructs a Bayesian network, given the contingency table for each relationship chain. Dashed lines: The absence and presence of Bayesian network edges learned for a shorter relationship chain are propagated as constraints for learning for a longer relationship chain.}
 \label{fig:lattice}
 %original is in figures/graphs.pptx
\end{figure*}
...
\KwIn{Database $\D$; parametrized random variables $\F$; relationship chain lattice $\lattice$ with maximum chain length $\it{m}$.}
\KwOut{A Bayes multi-net $\B_{\set{\Relation}}$ for relationship chains in $\lattice$.\\} 
...
\end{algorithmic}
\begin{algorithmic}[1]
\FORALL[compute BN for each entity type]{entity type $E$} 
\STATE{$\dtable_{E}$ := the contingency table for the attribute nodes of $E$} %\commment{test}
\STATE $\B_{E} := \it{BNL}(\dtable_{E},\emptyset)$
\ENDFOR
\FORALL[compute BN for each relationship node]{relationship node $\rnode$} 
\STATE{Find $\it{constraints}_{R}$ propagated from entity BNs} \COMMENT{Constraint~\ref{clause:econstraints}}
\STATE{$\dtable_{R} := \cttable(\rnodes{R}$} \COMMENT{the contingency table for the nodes associated with $R$}
\STATE $\B_{R} := \it{BNL}(\dtable_{R},\it{constraints}_{R})$
\ENDFOR
\FOR {chain length $\ell \leftarrow 2,3, \ldots, m$}
\FOR {each chain $\set{\Relation}$ of length $\ell$}
\STATE{Find $\it{constraints}_{\set{\Relation}}$ propagated from shorter chains $\set{\Relation}^{*} \subset \set{\Relation}$} \COMMENT{Constraint~\ref{clause:rconstraints}}
\STATE{$\dtable_{\set{\Relation}} := \cttable(\rnodes{\set{\Relation}}$} \COMMENT{the contingency table for the nodes associated with $\set{\Relation}$}
...
\subsection{The Lattice of Relationship Chains} 
...
\end{equation}
 ...
 A relationship chain $\set{\Relation}$ is a \defterm{subchain} of another chain $\set{\Relation'}$, written $\set{\Relation} \sqsubseteq \set{\Relation'}$, if every relationship par-RV in $\set{\Relation}$ occurs also in $\set{\Relation'}$. For example, the chain~(\ref{eq:rchain1}) is a subchain of the chain~(\ref{eq:rchain2}).
Two chains are equivalent in case they contain the same relationship variables. 
The \defterm{relationship lattice} contains a representative chain from each equivalence class. A representative chain for a set of relationship variables can be generated using any fixed order on relationship variables. In the following we do not distinguish between a relationship chain and its equivalence class unless there is risk of confusion.  The
...
\subsection{The Bayesian Multinet}
...
\begin{constraint} \label{clause:rconstraints} Suppose that nodes $\functor(\terms),g(\terms')$ appear in the variables $\rnodes{\set{\Relation}}$. %associated with relationship chain $\set{\Relation}$.
...
{\em Constraint~\ref{clause:econstraints}.} The Bayes net for the entity type $\it{Professor}$ contains the edge $\it{Popularity(P)} \rightarrow \it{Teaching\_ability(P)}$. Therefore the length 1 chain $\it{RA(P,S)} $ is {\em required} to contain this edge as well. The Bayes net graph for the entity type $\it{Course}$ does not contain the edge $\it{Difficulty(C)} \rightarrow \it{Level(C)}$, so the Bayes net for the length 1 chain $\it{Registered(S,C)}$ must {\em not} contain the edge $\it{Difficulty(C)} \rightarrow \it{Level(C)}$.

{\em Constraint~\ref{clause:rconstraints}.} The Bayes net for the length 1 chain $\it{RA(P,S)}$ contains an edge $\it{Salary(P,S)} \rightarrow \it{Capability(P,S)}$, and does not contain $\it{Popularity(P)} \rightarrow \it{Salary(P,S)}$. So for the length 2 chain $\it{Registered(S,C)},\it{RA(P,S)}$, the edge $\it{Salary(P,S)} \rightarrow \it{Capability(P,S)}$ is required. The edge $\it{Difficulty(C)} \rightarrow \it{Level(C)}$ is forbidden.  We next discuss how \FB leverages SQL to implement the LAJ algorithm. 
...
\caption{Trace of the learn-and-join hierarchical structure learning method of Figure~\ref{fig:lattice} for the University domain. The trace is shown for 
the $\it{RA}$ relationship only. For each relationship chain, the figure shows the complete Bayesian network structures learned, and excerpts from the contingency tables. Notice that the edges learned for the Professor table and for the Student table are propagated to the Bayesian network for the RA table.}
 \label{fig:lja-trace}
 %original is in figures/graphs.pptx
\end{figure*}

%\todo{change Registered(S,C) by RA(P,S) in Figure~\ref{fig:lja-trace}. Also for the caption.}
...
%Algorithm~\ref{alg:laj-pseudocode} shows how the tables are used to support structure learning. 
...
\resizebox{0.5\textwidth}{!}{
\begin{tabular}[c]{ccc}
  \multicolumn{2}{c}{\it{\textbf{LatticeMember}}}\\
\multicolumn{1}{c}{\textbf{Member}} &\multicolumn{1}{c}{\textbf{ Chain }}& \\\hline
$\it{RA}(\P,\S)$ & [$\it{RA}(\P,\S)$, $Registered(\S,\C)$ ]\\
 $Registered(\S,\C)$&[$\it{RA}(\P,\S)$, $Registered(\S,\C)$ ]\\\hline 
&&\\
\multicolumn{2}{c}{\it{\textbf{LatticeOrder}}}\\
\multicolumn{1}{c}{\textbf{Subchain}} & \multicolumn{1}{c}{\textbf{SuperChain}} \\\hline
$\it{RA}(\P,\S)$& [$\it{RA}(\P,\S)$, $Registered(\S,\C)$ ]\\ 
 $Registered(\S,\C)$ & [$\it{RA}(\P,\S)$, $Registered(\S,\C)$ ]\ \\\hline 
\end{tabular}
}
...
The lattice tables support SQL access to the internal structure of a relationship chain: The \dbtable{LatticeMember} table lists, for each valid relationship chain, the relationship nodes that are members of this chain. The \dbtable{LatticeOrder} table lists for each relationship chain, its immediate subchain. Table~\ref{tab:lattice-table} shows the lattice tables for our running example. %Here $\mathit{rchain}$ is a custom ID type for relationship chains. 
...
\resizebox{0.5\textwidth}{!}{
\begin{tabular}[c]{lll}
\multicolumn{3}{c}{\textbf{ChainBayesNets}} \\
\multicolumn{1}{c}{\textbf{Rchain }}& \multicolumn{1}{c}{\textbf{child}} & \multicolumn{1}{c}{\textbf{parent}} \\\hline
 \it{RA(P,S)} & Capability(P,S) &  \it{RA(P,S)} \\
 \it{RA(P,S)} & Intelligence(S) &  \it{RA(P,S)} \\
 \it{RA(P,S)} & Popularity(P) &  \it{RA(P,S)} \\
 \it{RA(P,S)} & Popularity(P) &Teachingability(P) \\
 \it{RA(P,S)}& Ranking(S) & Intelligence(S) \\
 \it{RA(P,S)} & Salary(P,S) &  \it{RA(P,S)} \\
 \it{RA(P,S)} & Salary(P,S) & Capability(P,S) \\\hline 
 &&\\
\multicolumn{3}{c}{\textbf{EntityBayesNets}}  \\
\multicolumn{1}{c}{\textbf{FOVariable}} & \multicolumn{1}{c}{\textbf{child}} & \multicolumn{1}{c}{\textbf{parent}}\\\hline
Prof & Popularity(P) & Teachingability(P) \\
Student & Ranking(S) & Intelligence(S) \\\hline
\end{tabular}
}
\end{center}
\label{table:path-bn}
...
For each relationship node (e.g. RA(P,S)), the \dbtable{Required Edges} are the edges learned for the associated FOVariables (contained in the $\it{EntityBayesNets}$ table). For each relationship chain, the required edges are the union of learned edges for shorter chains. Similarly for \dbtable{Forbidden Edges}. Required and forbidden edges are exported as constraints for Bayesian network learning. Table~\ref{table:constraint-edges} illustrates this representation.




\begin{table}[htp]
\caption{Required and Forbidden edges for the relationship chain RA(P,S). Horizontal lines separate constraints for different relationship chains. $P$ ranges over professors, $S$ over students, $C$ over courses.}
\begin{center}
\begin{tabular}{lll}
\multicolumn{3}{c}{\textbf{RequiredEdges}} \\
\multicolumn{1}{c}{\textbf{Rchain }}& \multicolumn{1}{c}{\textbf{child}} & \multicolumn{1}{c}{\textbf{parent}} \\\hline
 \it{RA(P,S)} & Popularity(P) & Teachingability(P) \\
 \it{RA(P,S)} & Ranking(S) & Intelligence(S) \\\hline
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)} 
\end{tabular} & Capability(P,S)&Salary(P,S)\\
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)} 
\end{tabular} & Popularity(P) & Teachingability(P)\\
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)} 
\end{tabular}& Popularity(P) & Teachingability(P)\\\hline\hline
&& \\
\multicolumn{3}{c}{\textbf{ForbiddenEdges} }\\

\multicolumn{1}{c}{\textbf{Rchain }}& \multicolumn{1}{c}{\textbf{child}} & \multicolumn{1}{c}{\textbf{parent}} \\\hline

 \it{Reg.(S,C)} & Level(C)& Difficulty(C) \\
 \it{Reg.(S,C)} &Difficulty(C)&   Level(C)\\\hline
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)}
\end{tabular} & Capability(P,S)&Salary(P,S)\\
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)}
\end{tabular}& Level(C)& Difficulty(C) \\
\begin{tabular}{l}
 \it{Reg.(S,C)},\\
 \it{RA(P,S)}
\end{tabular}& Difficulty(C) &  Level(C)\\ \hline
\end{tabular}
\end{center}
\label{table:constraint-edges}
\end{table}%
...
%
The new problem is finding the target contingency table. SQL allows us to solve this easily by restricting counts to the target entity in the WHERE clause. To illustrate, suppose we want to modify the contingency table query of Table~\ref{tab:meta-query} to compute the contingency table for $\S = jack$. We add the student id to the SELECT clause, and the join condition $S.s\_id = jack$ to the WHERE clause; see Table~\ref{table:target-query}. %\footnote{Omit apostrophes for readability.} 
...
Our experimental study describes how \FB  can be used to implement a challenging machine learning application: Constructing a Bayesian network model for a relational database. Bayesian networks are a good illustration of typical challenges and how RDBMS capabilities can address them because: (1) Bayesian networks are widely regarded as a very useful model % class
 in machine learning and AI, that supports decision making and reasoning under uncertainty. At the same time, they are considered challenging to learn from data. (2) Database researchers have proposed Bayesian networks for combining databases with uncertainty%\cite{Wang2008,Deshpande_VLDB2007}
...
{\em Limitations of pre-counting.}
...
While \FB  provides good solutions for each of these system capabilities in isolation, the ease with which large complex statistical-relational objects can be integrated via SQL queries is a key feature. 
...
%While our implementation has used simple SQL plus indexes, t
...
\resizebox{0.5\textwidth}{!}{
    \begin{tabular}{|r|r|r|r|r|r|}
    \hline
    \multicolumn{2}{|c|}{\textbf{Table Name}} & \multicolumn{4}{c|}{\textbf{Column Names} } \\
    \hline
...
% \section{MySQL script for creating the Random Variable Database}  \label{sec:appendix}
...
\bibliography{master}   % name your BibTeX data base
