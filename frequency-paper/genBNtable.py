"""
    ...
    random selection semantics from a database. Generates two LaTeX files, one a table
    specifying all the conditional and joint probabilities, together with the log-likelihood,
    the other file a short macro containing the pseudo-likelihood for the entire table.

    If you just wish to generate a table, you only need to modify the defitions given
    following the comment, "The system that we are analyzing". This includes modifying
    the function "formatJointTableForLaTeX" to get the LaTeX format your paper needs. Yo
    only need to look at the code preceding the comment if you want to modify the computations.
...
class Rule (object):
    """ A rule specifying a conditional probability (this class is likely misnamed) """
...
class BayesNet (object):
    """ The Bayes net """
    def __init__(this, uniformDefault=False):
        """
        ...
        """
        this.nodes = []
        this.uniformDefault = uniformDefault
...
def ruleMatch (ruleSet, node, parents):
    """
    Locate the value for a grounded node and its parents in a rule set, return NO_PROB if not found.
    ...
    prob = getProb (node)
    if prob == NO_PROB and functorRangeSize(node.functor) == 2:
        tn = copy.copy(node)
...
`   Given a joint probability table, format it for LaTeX.
    ...
    with open(MPLE_FILE,"w") as plogp:
        plogp.write("\\newcommand{\\pseudologp}{"+"{:.2f}".format(sum(logps)/len(logps))+"}\n")

# Name of LaTeX file to create with the table
TABLE_FILE = "table1.tex"
# Name of LaTeX file to contain the maximum pseudo-likelihood estimator
MPLE_FILE = "tab1plogp.tex"
...
ruleList = []
ruleList.append(Rule(Node ('g', ['Y'], 'M'), [], 0.5))
ruleList.append(Rule(Node ('F', ['X', 'Y'], 'T'), [], 0.1))

ruleList.append(Rule(Node('g', ['X'], 'W'), [Node('g', ['Y'], 'W'), Node('F', ['X', 'Y'], 'T')], 0.7))
ruleList.append(Rule(Node('g', ['X'], 'M'), [Node('g', ['Y'], 'M'), Node('F', ['X', 'Y'], 'T')], 0.7))
ruleList.append(Rule(Node('g', ['X'], 'M'), [Node('g', ['Y'], 'M'), Node('F', ['X', 'Y'], 'F')], 0.5))
ruleList.append(Rule(Node('g', ['X'], 'W'), [Node('g', ['Y'], 'W'), Node('F', ['X', 'Y'], 'F')], 0.5))

ruleList.append(Rule(Node ('cd', ['X'], 'T') , [Node('g', ['X'], 'M')], 0.6))
ruleList.append(Rule(Node ('cd', ['X'], 'T') , [Node('g', ['X'], 'W')], 0.8))
