...
We follow the well-established Exceptional Model Mining (EMM) framework, which has been previously applied for subgroup discovery in  propositional data; our novel contribution is to develop EMM for relational data. EMM leverages machine learning models for exception mining: A object is exceptional to the extent that a model learned for the object data differs from a model learned for the general population. In relational data, EMM can therefore be used for detecting single outlier or exceptional objects. We apply EMM with state-of-the-art statistical-relational model discovery methods that construct a graphical model (Bayesian network), that compactly represents probabilistic associations in the data. We investigate several outlierness metrics, based on the learned object-relational model, that quantify the extent to which the association pattern of a potential outlier object deviates from that of the whole population. 
...
Empirical evaluation compares EMM and aggregation-based approaches to relational outlier detection, with respect to three synthetic and four real-world datasets.
				...
				%For an outlier detection survey please see~\citep{aggarwal2013}.
...
%
The Opta dataset represents information about Premier League data %\citep{opta-original} 
(Sec.~\ref{sec:real}). 
...
%				\end{figure}
			...
			%The learn-and-join algorithm is the state-of-the-art Bayes net learning method for relational data, based on model search in the lattice of relationship joins \citep{Schulte2012}. The LAJ algorithm takes as input a relational database and outputs a Parametrized Bayes net structure.
...
\todo{maybe merge with the previous proposition}
...
\begin{table}[htbp]
	...
	\end{table}
				...
				%						The IMDb website offers a dataset containing information on cast, crew, titles, technical details and biographies in a set of compressed text files \citep{Peralta2007}.
				...
				\begin{enumerate}
					...
					%\item Internal Evaluation: in this type of evaluation outlier score has been used to quantify the extremity of data points~\citep{Pickands1975}.% We do not use this method of evaluation in our work.
				\end{enumerate}
...
\paragraph{Likelihood-based Outlier Scores.} The first approach evaluates the likelihood-based outlier scores described in Section~\ref{sec:metrics}. For relational Bayesian network structure learning we utilize the previous learn-and-join algorithm (LAJ), which is
					...
					%							\item[$\outrank$] employs subspace analysis to measure the degree of outlierness. It compares clusters in different subspace to derive an outlier score for each object \citep{Muller2012}. \end{LaTeXdescription}
...
%results in the same distance from the center of normal community for each individual.
...
%	auction appear to have been based on intangibles that are hard to quantify%
	\paragraph{Salary} is probably the most obvious, and at the same time often the most misleading way to measure success of the players. Previous studies suggest that salary of the players does not  always follow their performance in many sports such as Baseball and Soccer~\citep{Hall2002,Barrio2004}. They show that pay cannot be explained only by past performance and there are other factors that are hard to quantify and have great effects on the salaries. 
	 ...
	 Figure~\ref{fig:strikersELD} plots $\mid$ against striker success metrics. We observe a large cluster around the origin, which points to a large base of normal strikers with both salaries and low $\mid$ scores.\todo{what is the scale of salary? E.g. annual} The striker with the greatest $\mid$ score is Robin van Persie. He stands out in terms of Shots on Target, Time Played, and Salary. 
	...
	Figure~\ref{fig:goaliesELD} shows the correlation of $\mid$ with Goalie success metrics in a scatter plot. Goalies do not vary much in terms of the time they play. Wayne Hennessey has the highest number of Saves Made and also an unusually high $\mid$ score, although not the highest.\todo{what is the time scale? Total over Season?}
	...
	%	\subfigure[Teams: Team Standing vs. $\mid$ for the top teams in Premier League. \textbf{separate figure please}]{\label{fig:TeamStanding}\includegraphics[width=0.5\textwidth]{topTeamStats.pdf}}
		\caption{Correlations with Success Metrics for Goalies.\label{fig:goaliesELD}}
	\end{figure}
...
The appendix contains the proofs for the results about $f$-divergences (Proposition~\ref{prop:xi-square}). We show that if the generator $f$ is twice differentiable at $u=1$, the resulting $f$-divergence is approximately a scaled $\chi^{2}$ divergence. This result does not apply to the ELD divergence whose generator is $f(u) = |ln(u)|$ \todo{check the generator}. The ELD divergence instead approximates the total variation distance. \todo{maybe state Jensen's inequality} 
