...
% ~\cite{SonderbyLadderVAE16,kingma2013auto}. 
...
\begin{figure}[htbp]
    ...
    \end{minipage}%
    \caption{Visualizing player embedding with T-SNE. Each data point corresponds to precisely a player embedding conditioning on the game context at the current %event (or 
    time step $t$.  
...
% The embeddings computed by our \system (top plots) show a {\it shrinkage effect}. 
...
% including expected goals estimation and score difference prediction.
...
\hspace{-0.2in}
    ...
    \begin{tabular}{c|ccc}
        \toprule
        \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}} Embedding\\ Method\end{tabular}} & \multicolumn{3}{c}{Performance} \\ \cline{2-4}
        ...
        CVAE & 0.33  $\pm$  5.33E-5 & 0.95  $\pm$  8.72E-5 & 0.49  $\pm$  8.29E-5 \\
        MA-BE & 0.35  $\pm$  1.44E-4 & 0.91  $\pm$  2.30E-4 & 0.50  $\pm$  1.56E-4 \\
        CAERNN & 0.29  $\pm$  1.05E-4 & 0.96  $\pm$  1.56E-4 & 0.44  $\pm$  1.65E-4 \\
        CVRNN  & {\bf 0.40}  $\pm$  4.75E-4 & 0.84  $\pm$  1.81E-4 & 0.54  $\pm$  2.97E-4 \\ 
        \system & 0.37  $\pm$  2.01E-4 & {\bf 0.98}  $\pm$  1.32E-4 & {\bf 0.54}  $\pm$  8.14E-5 \\\bottomrule
    \end{tabular}
    }
    \caption{The results for expected goal estimation 
    % Results for predicting the Expected Goal 
    applying the examined player representation methods. 
    % The evaluation metrics include Precision (P), Recall (R) and F1-score.
    }
    \label{table:exp-eg}
    \vspace{-0.05in}
...
% demonstrates the effectiveness of our contextualized representation framework and hierarchical model structure.
% optional further points: representations correlate with postion (offensive vs. defensive) and with number of shots, which both correlate with shooting ability
% , which allows it accurately generalizes to different players and game contexts.
...
\begin{minipage}[b]{.28\textwidth}
    \centering
    \subfloat{{\includegraphics[width=1\textwidth]{./figures/temporal-absolute-difference-plot.png}}}%
    % \quad
    % \subfloat{{\includegraphics[width=0.45\columnwidth]{./figures/temporal-absolute-difference-shadow-plot.png} }}%
    \captionsetup{width=1\linewidth}
    \captionof{figure}{Temporal illustrations of the absolute error between predicted and final score differences.
