...
/**Modified by Jeff Cleveland for use in RPTViewer
 * Changed the size of layoutRectangle in setScrollBars
 * to actually encompass the graphs we generate.
 *
 * Due to setScrollBars being private simple extending the
 * class would not do and we duplicated the class.
 */
...
public class Scroll extends JPanel {
    ...
    final static int SCROLL_OFFSET = 480;
     ...
     * @param vv
     */
    public Scroll(VisualizationViewer vv) {
    ...
    private void setScrollBars(VisualizationViewer vv) {
        Dimension d = ((DecisionTreeLayout) vv.getGraphLayout()).getSize();
        d.width *= 2;

        Rectangle vvBounds = vv.getBounds();

        Rectangle layoutRectangle =
                new Rectangle((int) (-d.width / 2) + SCROLL_OFFSET, 0, d.width, d.height);
