/**
 * $Id: StochasticModeEstimator.java 3658 2007-10-15 16:29:11Z schapira $
 *
...
/**
 * This estimator adds a random value from each subgraph to the distribution.
 * In order to estimate the probability per subgraph, it gets the probability of a random value per subgraph.
 ...
 * 2  c
 * <p/>
 * then prob. distribution could be is {b:50%, c:50%}. Also, if the subgAttrNSt is
 * <p/>
 * * subg_id value
 * ------- -----
 * 1  a
 ...
 * 2  d
 * <p/>
 * then the resulting probabilities0per subgraph could be:
 * <p/>
 ...
 * <p/>
 * 1 -> 0.5   (known b)
 * 2 -> 0.005 (unknown d / 2 possible values)
 */
public class StochasticModeEstimator implements Estimator {

    private static Logger log = Logger.getLogger(StochasticModeEstimator.class);
    ...
    public void recordObservations(NST attrNST, ProbDistribution observedProbDist) {
        NST aggregate = attrNST.aggregate("rand_row", "subg_id", "value");
    ...
    public HashMap<String, Double> getSmoothedProbsGivenAttribute(NST subgAttrNST, ProbDistribution observedProbDistr, int randSeed) {
        ...
        NST aggregateNST = subgAttrNST.aggregate("rand_row", "subg_id", "value");
