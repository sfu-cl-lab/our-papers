/**
 * $Id: KeyInFilterTest.java 3658 2007-10-15 16:29:11Z schapira $
 *
 ...
 */
public class KeyInFilterTest extends TestCase {
    ...
    public void testApplyFindsWrongKey() throws Exception {
        ...
        int foundRows = MonetUtil.getRowCount(resultBat);
        if (foundRows != 0) {
            fail("Expected 0 rows for key=4, found " + foundRows);
        }
        Connection.releaseSavedVar(resultBat);
    }
    ...
    public void testApplyFindsExistingKeyFromBAT() throws Exception {
        filterNST.insertRow("1");
        Filter filter = new KeyInFilter(filterNST.getNSTColumn("oid").getBATName());
        String resultBat = Connection.executeAndSave(filter.getApplyCmd(nst));
        int foundRows = MonetUtil.getRowCount(resultBat);
        if (foundRows != 1) {
            fail("Expected 1 rows for key=1, found " + foundRows);
